...
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
  ...
  <script>document.addEventListener("DOMContentLoaded",function(){if(window.AM_LOGO_B64){["login-logo","loading-logo"].forEach(function(id){var el=document.getElementById(id);if(el)el.src=window.AM_LOGO_B64;});}})</script>
</head>
<body>

<div id="loading-screen">
  <img id="loading-logo" alt="AM BADMINTON" style="width:260px;max-width:75vw;margin-bottom:18px;">
...
<div id="login-page">
  ...
  <div class="login-container">
    <div class="login-hero">
      <img id="login-logo" alt="AM BADMINTON" style="width:300px;max-width:85vw;margin-bottom:8px;">
    </div>
    <div class="login-card">
      ...
      <div class="login-panel" id="panel-signup">
        ...
        <!-- 개인정보 동의 -->
        <div style="margin-bottom:12px;padding:12px;background:var(--bg3);border-radius:10px;border:1px solid var(--border);">
...
<div id="app">
  <header class="app-header">
    <div class="app-header-logo" onclick="goHome()">🏸 새벽민턴</div>
  ...
  <div class="app-body">
    ...
    <div class="page" id="page-feed">
      <div class="flex-between mb-2">
        ...
        <div style="display:flex;gap:6px;align-items:center;">
          ...
          <button class="btn btn-primary btn-sm" onclick="openRegisterModal()" style="font-size:.82rem;">✍️ 등록</button>
        </div>
      </div>
    ...
    <div class="page" id="page-admin">
      ...
      <div class="sub-tabs">
        ...
        <button class="sub-tab" onclick="switchAdminTab('tournamentImport')">🏆 대회입력</button>
        <button class="sub-tab" onclick="switchAdminTab('matchDelete')">🗑️ 경기삭제</button>
      </div>
    ...
    <div class="page" id="page-tournament">
      <div class="flex-between mb-2">
        ...
        <div style="display:flex;gap:6px;">
          <button id="btn-bulk-import" class="btn btn-ghost btn-sm" onclick="toggleBulkImportForm()" style="display:none;">📋 일괄입력</button>
          <button id="btn-balance" class="btn btn-ghost btn-sm" onclick="toggleBalanceForm()" style="display:none;">⚖️ 밸런스</button>
          <!-- 대회 만들기 버튼 미사용 -->
        </div>
      </div>

      <!-- ── 일괄입력 폼 (관리자 전용) ── -->
      <div id="bulk-import-inline" style="display:none;" class="card mb-3"></div>

      <!-- ── 밸런스 비교 폼 (관리자 전용) ── -->
      <div id="balance-form-inline" style="display:none;" class="card mb-3">

        <!-- 헤더 -->
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
          <div>
            <div style="font-weight:700;font-size:.95rem;">⚖️ 밸런스 비교</div>
            <div style="font-size:.74rem;color:var(--text-muted);margin-top:2px;">참석자 실력 기반 균형 배분 검토</div>
          </div>
          <button onclick="toggleBalanceForm()" style="background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:1.1rem;padding:4px 8px;">✕</button>
        </div>
        ...
        <div id="bal-step1">
          ...
          <div style="margin-bottom:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
              <div style="font-size:.8rem;font-weight:700;">참석자 선택</div>
              <div id="bal-attendee-count" style="font-size:.74rem;color:var(--text-muted);">0명 선택됨</div>
            </div>
            <div id="bal-attendee-list" style="display:flex;flex-wrap:wrap;gap:5px;min-height:44px;background:var(--bg2);border-radius:10px;padding:10px;"></div>
          </div>

          <!-- 종목 -->
          <div style="margin-bottom:12px;">
            <div style="font-size:.8rem;font-weight:700;margin-bottom:6px;">종목</div>
            <div style="display:flex;gap:6px;">
              <button id="bal-type-individual" class="btn btn-primary btn-sm" onclick="balSetType('individual')" style="flex:1;">👤 개인전</button>
              <button id="bal-type-duo"        class="btn btn-ghost  btn-sm" onclick="balSetType('duo')"        style="flex:1;">👥 듀오전</button>
              <button id="bal-type-team"       class="btn btn-ghost  btn-sm" onclick="balSetType('team')"       style="flex:1;">🚩 팀장전</button>
            </div>
          </div>

          <!-- 팀장전 팀장 지정 -->
          <div id="bal-captain-section" style="display:none;margin-bottom:12px;">
            <div style="font-size:.78rem;color:var(--text-muted);margin-bottom:6px;">팀장 지정</div>
            <div style="display:flex;gap:8px;">
              <div style="flex:1;">
                <div style="font-size:.74rem;color:#00C896;margin-bottom:3px;">A팀 팀장</div>
                <select class="form-select" id="bal-captain-a" style="font-size:.82rem;"><option value="">선택</option></select>
              </div>
              <div style="flex:1;">
                <div style="font-size:.74rem;color:#FF7043;margin-bottom:3px;">B팀 팀장</div>
                <select class="form-select" id="bal-captain-b" style="font-size:.82rem;"><option value="">선택</option></select>
              </div>
            </div>
          </div>

          <!-- 듀오전 페어 구성 -->
          <div id="bal-duo-section" style="display:none;margin-bottom:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
              <div style="font-size:.78rem;font-weight:700;">파트너 구성</div>
              <div style="font-size:.72rem;color:var(--text-muted);">선수 클릭→첫번째, 다시 클릭→파트너</div>
            </div>
            <div id="bal-duo-pairs-wrap"></div>
          </div>

          <button class="btn btn-primary" style="width:100%;" onclick="balGenerate()">⚖️ 밸런스 분석</button>
        </div>

        <!-- STEP 2: 결과 + 수정 -->
        <div id="bal-step2" style="display:none;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            <div style="font-size:.82rem;font-weight:700;">배분 결과</div>
            <button onclick="balGoStep(1)" style="font-size:.75rem;padding:3px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:8px;cursor:pointer;color:var(--text-muted);">← 다시</button>
          </div>

          <!-- 밸런스 점수 게이지 -->
          <div id="bal-gauge" style="background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:10px;"></div>

          <!-- 수정 영역 -->
          <div id="bal-edit" style="margin-bottom:10px;"></div>

          <!-- 저장 -->
          <button class="btn btn-primary" style="width:100%;" onclick="balSave()">💾 이 배분 저장</button>
        </div>

      </div><!-- /balance-form-inline -->

      <!-- 대회 만들기 폼(bracket-form-inline) 미사용 -->
